...
from .dcnv3 import build_deformable_conv3,build_dcn,DCNv3_pytorch
from .S3_DSConv import DSConv
from .flatten_swin import FocusedLinearAttention
# from yolov6.models.reppan import RepPANNeck6,RepPANNeck6_q,RepPANNeck6_qq,RepPANNeck6_qq_fen_zu
...
class SparseRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # self.backbone_dcn = build_deformable_conv3()
        # self.backbone_p1 = build_deformable_conv3()
        # self.backbone_p2 = build_deformable_conv3()
        # self.backbone_p3 = build_deformable_conv3()
        # self.backbone_p4 = build_deformable_conv3()
        # self.back_l = [self.backbone_p1,self.backbone_p2,self.backbone_p3,self.backbone_p4]
        # self.mix1 = MixStructureBlock(256)
        # self.mix2 = MixStructureBlock(256)
        # self.mix3 = MixStructureBlock(256)
        # self.mix4 = MixStructureBlock(256)
        # self.mix1 = nn.Sequential(build_deformable_conv3(),MixStructureBlock(256))
        # self.mix2 = nn.Sequential(build_deformable_conv3(),MixStructureBlock(256))
        # self.mix3 = nn.Sequential(build_deformable_conv3(),MixStructureBlock(256))
        # self.mix4 = nn.Sequential(build_deformable_conv3(),MixStructureBlock(256))
        # self.back_l = [self.mix1,self.mix2,self.mix3,self.mix4]
        # self.mix = nn.Sequential(MixStructureBlock(256))
    ...
    def forward(self, batched_inputs):
        ...
        #使用相同的卷积核
        # features = [self.backbone_dcn(feature) for feature in features]
        ...
        for f,p in zip(features,self.back_l):
            ff.append(p(f))
        features = ff
        # features = [self.mix(feature) for feature in features]
...
class MixStructureBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.mixa= MixAttentionStructureBlock(dim=256)
    ...
    def forward(self, x):
        ...
        x = self.mixa(x)
        return x
class MixAttentionStructureBlock(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dw1 = nn.Conv2d(dim, dim, kernel_size=5, padding=2,groups=dim )
        self.dw2 = nn.Conv2d(dim, dim, kernel_size=3, padding=1,groups=dim )
        self.dw3 = nn.Conv2d(dim, dim, kernel_size=1, groups=dim )
    ...
    def forward(self,x):
        identity = x
        x = torch.cat([identity*self.sigmoid1(self.gelu1(self.dw1(x))), identity*self.sigmoid2(self.gelu2(self.dw2(x))),identity*self.sigmoid3(self.gelu3(self.dw3(x)))], dim=1)
        x = self.conv(x)
        x = identity + x
        return x
